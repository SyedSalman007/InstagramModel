...
class PriorityNode
{
	...
	public:		
		PriorityNode* next;
		PriorityNode(string priority, string data)
		{
...
class PriorityQueueList
{
	public:	
		...
		void make_list(string file_name)
		{
			...
			fstream file_read;
			string index;
			string line = "", new_line = "", new_line2 = "";
			int length;			
			file_read.open(file_name.c_str(), ios::in | ios::app);			
			if(file_read.fail())
			{
			...
			for(int i = 0; i < length; i++)
			{
				PriorityNode* temp = new PriorityNode(pri[i], post[i]);
				insert_list(root, temp);
			}				
			while(getline(file_read, line))	//       Getting the friends text file:
			{
				stringstream ss(line);
				getline(ss, index, ',');
				buddy_file(index);
		...
		void my_post(string file_name)
		{
			...
			{
				PriorityNode* temp = new PriorityNode(pri[i], post[i]);
				insert_my_list(searched_user, temp);
			}
			my_display();
		}
		void insert_my_list(PriorityNode* temp_root, PriorityNode* temp)
		{	
			if(temp_root == NULL)
			{
				searched_user = temp;
			...
			{
				if(temp_root->get_priority() < temp->get_priority())
				{
					temp->next = temp_root;
					searched_user = temp;
				}
				...
				{
					while(temp_root->next != NULL && temp_root->next->get_priority() > temp->get_priority())
					{
						temp_root = temp_root->next;
					}
					temp->next = temp_root->next;
					temp_root->next = temp;
				}
			}
		}
		void buddy_file(string file)
		{
			FriendNode* temp = new FriendNode(file);
			FriendNode* p = node;	
			if(p == NULL)
			{
				node = temp;
		...
		void insert_list(PriorityNode* temp_root, PriorityNode* temp)
		{
			if(temp_root == NULL)
			{
				root = temp;
		...
		void insert_friend()
		{
			...
			while(temp != NULL)
			{
				string filename = temp->file;	//			cout<<filename;
				fstream file_read;
				string index, value;
				string line = "";
				int length;				
				file_read.open(filename.c_str(), ios::in | ios::app);
				if(file_read.fail())
				{
					cout<<"\nFile failed to opened in friend priority queue\n";
					exit(1);
				}				
				getline(file_read, line);
				stringstream ss(line);
				...
				PriorityNode* temp1 = new PriorityNode(index, value);
				if(friend_root == NULL)
				{
					friend_root = temp1;
				}
		...
		void my_display()
		{
			PriorityNode* temp = searched_user;
...
//			string search;
//			cin>>search;
//    		node<string>* data = t.searchstring(search);
//    		filename = data->key;
//    		if(data != NULL)
//    		{
//    			fstream file;
...
void AVL<T> :: searchup(string user_file_name)
{
	...
	do
	{
		...
		PriorityQueueList user;
//		user.make_list(filename); 
//		user.my_post(filename);
		fstream file;
